...
def test_get_restaurant_by_id(db_session):
    ...
    assert data.content == {'id': '1', 'name': 'Cool place'}


def test_get_restaurant_by_bad_id(db_session):
    """
    ...
    assert data.content == {
        'message': 'restaurant with id 12345 was not found'}
...
def test_add_new_restaurant(db_session):
    ...
    query = db_session.query(RestaurantsModel).filter(
        RestaurantsModel.id == data.content['id']).first()
...
def test_put_update_restaurant_name_should_update_name(db_session):
    ...
    query = db_session.query(RestaurantsModel).filter(
        RestaurantsModel.name == 'Another place').all()
    assert len(query) == 1
...
def test_put_update_restaurant_name_should_return_NotFound_when_id_is_bad(db_session):
    ...
    assert data.status == 404
    assert data.content == {
        'message': 'unable to update restaurant 4 with name Another place'}
...
def test_put_update_restaurant_name_should_return_NotFound_name_is_empty(db_session):
    ...
    assert data.status == 400
    assert data.content == {
        'message': 'unable to update restaurant 3 with name '}

    query = db_session.query(RestaurantsModel).filter(
        RestaurantsModel.name == '').all()
    assert len(query) == 0
...
def test_get_menus_for_restaurant_should_return_menus(db_session):
    ...
    db_session.commit()
    db_session.add_all([
        MenusModel(id="2", name="my first menu",
                   description="The best first menu", restaurant_id="1"),
        MenusModel(id="3", name="my first menu",
                   description="The best first menu", restaurant_id="1"),
        MenusModel(id="4", name="my first menu",
                   description="The best first menu", restaurant_id="1")
    ])
    db_session.commit()
